...
using System.Linq;
using SPConsoleJson.ViewModel;
using System.Data.SqlClient;
using System.Data;
using Newtonsoft.Json;
using System.Collections.Generic;

namespace SPConsoleJson
{
    class Program
    {
        static void Main(string[] args)
        {
            using (EFContext context = new EFContext())
            {
                ...
                ReadDataEFContext(context, 0, 25000);
                Console.WriteLine("---------Read Storage-----------");
                GetDataSQLStorage(context, 0, 25000);
                Console.WriteLine("---------Read EFContext-----------");
                ReadDataEFContext(context, 0, 25000);
                Console.WriteLine("---------Read Storage-----------");
                GetDataSQLStorage(context, 0, 25000);
        ...
        static void GetDataSQLStorage(EFContext context, int skip, int take)
        {
            ...
            context.Database.ExecuteSqlCommand(sql,
                new SqlParameter("@From", Convert.ToInt32(skip)),
        ...
        static void ReadDataEFContext(EFContext context, int skip, int take)
        {
            ...
            query = query.OrderBy(u => u.UserId).Skip(skip).Take(take);
            int i = 1;
        ...
        static void SeedDB(EFContext context)
        {
            ...
            context.Users.AddRange(userGenerator.Generate(25000).ToArray());
            context.SaveChanges();
            ...
            for (int j = 0; j < 10; j++)
            {
                var imageGenerator = new Faker<UserImage>()
                                    ...
                                    .RuleFor(i => i.UserId, f => f.Random.Number(1, 25000));
